...
class WP_Test_WP_Job_Manager_Functions extends WPJM_BaseTest {
	...
	public function test_get_job_listings_location() {
		$locations = array( 'seattle' => array(), 'portland' => array(), 'oregon' => array(), 'london' => array() );
		$locations['seattle'][] = $this->factory->job_listing->create( array(
		...
		$locations['portland'][] = $this->factory->job_listing->create( array(
		...
		$locations['portland'][] = $locations['oregon'][] = $this->factory->job_listing->create( array(
			'post_title' => 'Dinosaur Test Portland-C',
			'meta_input' => array(
				'_job_location' => 'Portland, Oregon',
			),
		) );
		$locations['london'][] = $this->factory->job_listing->create( array(
		...
		$seattle_job_listings = get_job_listings( array( 'search_keywords' => 'Dinosaur', 'search_location' => 'Seattle' ) );
		$portland_job_listings = get_job_listings( array( 'search_keywords' => 'Dinosaur', 'search_location' => 'Portland' ) );
		$london_job_listings = get_job_listings( array( 'search_keywords' => 'Dinosaur', 'search_location' => 'London' ) );
	...
	public function test_get_job_listings_post_status() {
		$post_statuses = array( 'publish' => array(), 'expired' => array(), 'preview' => array() );
		$post_statuses['publish'][] = $this->factory->job_listing->create( array(
			'post_title' => 'Dinosaur Test Pub-A',
		) );
		$post_statuses['publish'][] = $this->factory->job_listing->create( array(
			'post_title' => 'Dinosaur Test Pub-B',
		) );
		$post_statuses['expired'][] = $this->factory->job_listing->create( array(
		...
		$post_statuses['preview'][] = $this->factory->job_listing->create( array(
			'post_title' => 'Dinosaur Test Preview-A',
			'post_status' => 'preview',
		) );

		$published_job_listings = get_job_listings( array( 'search_keywords' => 'Dinosaur', 'post_status' => array( 'publish' ) ) );
		$published_preview_job_listings = get_job_listings( array( 'search_keywords' => 'Dinosaur', 'post_status' => array( 'publish', 'preview' ) ) );
		$expired_job_listings = get_job_listings( array( 'search_keywords' => 'Dinosaur', 'post_status' => array( 'expired' ) ) );
	...
	public function test_get_job_listings_categories() {
		...
		$post_categories = array( 'empty' => array(), 'main' => array(), 'weird' => array(), 'all' => array() );

		$post_categories['main'] = $this->factory->job_listing->create_many( 3, array(
			'tax_input' => array(
				'job_listing_category' => $categories['main'],
			),
		) );
		$post_categories['weird'] = $this->factory->job_listing->create_many( 2, array(
			'tax_input' => array(
				'job_listing_category' => $categories['weird'],
			),
		) );
		$post_categories['happy'] = $this->factory->job_listing->create_many( 2, array(
			'tax_input' => array(
		...
		$results['jazz'] = array(
			...
			'results' => get_job_listings( array( 'search_keywords' => '', 'search_categories' => array( 'jazz' ) ) ),
		);
		$results['potato'] = array(
			'expected' => $post_categories['weird'],
			'results' => get_job_listings( array( 'search_keywords' => '', 'search_categories' => array( 'potato' ) ) ),
		);
		...
		$results['potato_coffee_some'] = array(
			...
			'results' => get_job_listings( array( 'search_keywords' => '', 'search_categories' => array( 'potato', 'coffee' ) ) ),
		);
		$results['potato_coffee_some'] = array(
			'expected' => $post_categories['main'],
			'results' => get_job_listings( array( 'search_keywords' => '', 'search_categories' => array( 'jazz', 'swim' ) ) ),
		);
	...
	public function test_get_job_listings_job_types() {
		...
		$tags = array( 'main' => array(), 'weird' => array(), 'happy' => array(), 'all' => array(), 'none' => array() );
		$terms = array();
		...
		$post_job_types = array( 'empty' => array(), 'main' => array(), 'weird' => array(), 'all' => array() );

		$post_job_types['main'] = $this->factory->job_listing->create_many( 3, array(
			'tax_input' => array(
				'job_listing_type' => $tags['main'],
			),
		) );
		$post_job_types['weird'] = $this->factory->job_listing->create_many( 2, array(
			'tax_input' => array(
				'job_listing_type' => $tags['weird'],
			),
		) );
		$post_job_types['happy'] = $this->factory->job_listing->create_many( 2, array(
			'tax_input' => array(
		...
		$results['jazz'] = array(
			...
			'results' => get_job_listings( array( 'search_keywords' => '', 'job_types' => array( 'jazz' ) ) ),
		);
		$results['potato'] = array(
			'expected' => $post_job_types['weird'],
			'results' => get_job_listings( array( 'search_keywords' => '', 'job_types' => array( 'potato' ) ) ),
		);
		$results['potato_coffee'] = array(
			'expected' => array_merge( $post_job_types['weird'], $post_job_types['happy'] ),
			'results' => get_job_listings( array( 'search_keywords' => '', 'job_types' => array( 'potato', 'coffee' ) ) ),
		);
		$results['potato_coffee'] = array(
			'expected' => $post_job_types['main'],
			'results' => get_job_listings( array( 'search_keywords' => '', 'job_types' => array( 'jazz', 'swim' ) ) ),
		);
	...
	public function test_get_job_listings_featured() {
		...
		$featured_flag['featured'] = $this->factory->job_listing->create_many( 3, array(
			'meta_input' => array(
				'_featured' => 1,
			),
		) );
		$featured_flag['not-featured'] = $this->factory->job_listing->create_many( 2, array(
			'meta_input' => array(
				'_featured' => 0,
			),
		) );
		...
		$results['featured'] = array(
			...
			'results' => get_job_listings( array( 'search_keywords' => '', 'featured' =>  true ) ),
		);
		$results['not-featured'] = array(
			'expected' => $featured_flag['not-featured'],
			'results' => get_job_listings( array( 'search_keywords' => '', 'featured' =>  false ) ),
		);
	...
	public function test_get_job_listings_featured_rand_cache() {
		...
		for ($i = 1; $i <= 10; $i++) {
			$results = get_job_listings( array( 'search_keywords' => '', 'orderby' => 'rand' ) );
			$tc = 0;
	...
	public function test_get_job_listings_filled() {
		...
		$featured_flag['filled'] = $this->factory->job_listing->create_many( 3, array(
			'meta_input' => array(
				'_filled' => 1,
			),
		) );
		$featured_flag['not-filled'] = $this->factory->job_listing->create_many( 2, array(
			'meta_input' => array(
				'_filled' => 0,
			),
		) );
		...
		$results['filled'] = array(
			...
			'results' => get_job_listings( array( 'search_keywords' => '', 'filled' =>  true ) ),
		);
		$results['not-filled'] = array(
			'expected' => $featured_flag['not-filled'],
			'results' => get_job_listings( array( 'search_keywords' => '', 'filled' =>  false ) ),
		);
